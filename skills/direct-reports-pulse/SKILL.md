...
8. Produce a compact Markdown report with exactly these sections:
   ...
   2. **Team snapshot** - opened PRs, merged PRs, merge rate, active contributors, merged PRs per active contributor, median time to merge, and P80 time to merge. Show the report week and trailing four weeks side by side in Markdown.
   ...
   4. **Weekly trends** - show opened PRs, merged PRs, merge rate, and median creation-to-merge for each of the four complete Monday-Sunday weeks. Use a compact Markdown table as the non-interactive fallback.
...
10. Render `direct-reports-pulse.html` as a self-contained interactive report:
    - Embed the filtered, identity-resolved, deduplicated four-week PR dataset as JSON in the HTML. Include only fields needed to recompute the report: source, stable PR key, author alias, repository, title, description, URL, created timestamp, merged/closed timestamp, status, target branch, chore classification, and creation-to-merge business hours. Escape embedded JSON so PR text cannot terminate the script element or inject markup.
    - Put a period selector at the top with the latest complete week, each of the other three complete weeks, and **All four weeks**. Default to the latest complete week.
    ...
    - Plot the four weekly values for opened PRs, merged PRs, merge rate, and median creation-to-merge in **Weekly trends**. Keep the four-week chart visible for every selection and clearly highlight the selected week or all-weeks range. Implement it with inline HTML, CSS, and JavaScript only; do not load scripts, fonts, or chart libraries from a network.
